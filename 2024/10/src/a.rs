...
impl ops::AddAssign<Self> for Score {
...
impl<'a> Hiker<'a> {
  ...
  fn depth_first_explore(&self, start: Point) -> Score {
    ...
    loop {
        ...
        self.next_adjacent_uphill(current_pos, DirectionRange::from(current_dir))
      {
        ...
        if adj_height == Height::MAX && !visited.contains(&adj_point) {
          score.increment();
          visited.insert(adj_point);
        }
