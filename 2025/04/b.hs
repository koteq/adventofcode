...
main = do
  ...
  let grid = parseGrid content
      countBefore = countRolls grid
      cleanedGrid = removeAccessibleRolls grid
      countAfter = countRolls cleanedGrid
  print $ countBefore - countAfter

removeAccessibleRolls :: Grid -> Grid
removeAccessibleRolls grid
  | removed > 0 = removeAccessibleRolls inaccessibleRolls
  | otherwise = grid
  where
    inaccessibleRolls = filterGrid isInaccessibleRoll grid
    removed = countRolls grid - countRolls inaccessibleRolls
...
isInaccessibleRoll :: Grid -> (Int, Int) -> Bool
isInaccessibleRoll (Grid grid) (x, y) =
  let isRoll = grid ! (x, y)
...
countRolls :: Grid -> Int
countRolls (Grid grid) = length $ filter (== True) (elems grid)
